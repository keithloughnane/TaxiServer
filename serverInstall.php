...
echo "INSTALL SCRIPT DELETE ONCE INSTALLED AND TESTED";
...
function recievedActivate($fromDeviceID,$mode) 								//This updates the active time so it doesn't timeout
{
	...
	echo "----------INSTALLING DATABASES-----------------";
			global $conn;
		$sqlStmnt = "	create table pickupCalc (calcId integer, pickupId integer, taxiDeviceId  varchar(20), distance double, offered dateTime)";
		$result1 = mysql_query($sqlStmnt,$conn);


	
			$sqlStmnt = "	create table devices (deviceId varchar(20) ,master bool,llong double, llat double, active bool, onPickupID integer, lastActive dateTime,available bool)";
		$result2 = mysql_query($sqlStmnt,$conn);


	
			$sqlStmnt = "	create table pickups (pickupId integer,taxiID  varchar(20),clientID  varchar(20),llong double, llat double,ETA dateTime,StartTime dateTime,completedTime dateTime,status int)";
		$result3 = mysql_query($sqlStmnt,$conn);




			$sqlStmnt = "CREATE TABLE `messages` (
  ...
  `delivered` datetime DEFAULT NULL
		...
		$result4 = mysql_query($sqlStmnt,$conn);
	}
...
create table pickupCalc (calcId integer, pickupId integer, taxiDeviceId integer, distance double, offered dateTime);

insert into pickupCalc (calcId, pickupId, taxiDeviceId, distance, skip)
values (1,1,1,1.0,false);

SELECT MIN(distance) FROM pickupCalc where pickupId = 4;

delete from pickupCalc where taxiDeviceId = 3 and pickupId = 23;

select deviceId, llong, llat from devices where master = true and active = true and available = true;

create table devices (deviceId integer,master bool,llong double, llat double, active bool, onPickupID integer, lastActive dateTime,available bool);

insert into devices(deviceId, master, llong, llat, active, onPickupID,available) values (1,true,345.678,423.545,true,0,true);

update devices set llong = 1.23, llat = 3.21 where deviceid = 0;

create table pickups (pickupId integer,taxiID integer,clientID integer,llong double, llat double,ETA dateTime,StartTime dateTime,completedTime dateTime,status int);

insert into pickups (pickupId, taxiID, clientID, llong, llat ,ETA ,StartTime ,completedTime ,status) values (1,1,1,345.678,423.54,'2000-01-01 13:00:30','2000-01-01 13:00:30','2000-01-01 13:00:30',1);

insert into pickups (pickupId, taxiID, clientID, llong, llat ,ETA ,StartTime ,completedTime ,status) values (1,1,1,345.678,423.54,'2000-01-01 13:00:30','2000-01-01 13:00:30','2000-01-01 13:00:30',1);
insert into pickups (pickupId, taxiID, clientID, llong, llat ,ETA ,StartTime ,completedTime ,status) values (MAX(pickupId),1,1,345.678,423.54,'2000-01-01 13:00:30','2000-01-01 13:00:30','2000-01-01 13:00:30',-1);
SELECT MAX(pickupId) FROM pickups

insert into messages (messageID,receiverID,msgText,status,created) values(1,1,"Hello",0,'2000-01-01 13:00:30');

select * from messages where receiverID = 1;

//insert into devices(id, other) values (1,"other");

//not ID 0 is used for testing only
-- Test Urls

127.0.0.1/server.php?msgtype=yep&args=yup
127.0.0.1/server.php?msgtype=setPosition&id=0&llong=123.456&llat=987.654
127.0.0.1/server.php?msgtype=requestPickup&id=0&llong=123.456&llat=987.654
127.0.0.1/server.php?msgtype=acceptPickup&id=0&pickupid=0
127.0.0.1/server.php?msgtype=refusePickup&id=2&pickupid=23
127.0.0.1/server.php?msgtype=activate&id=0
127.0.0.1/server.php?msgtype=deActivate&id=0
127.0.0.1/server.php?msgtype=getMessages&id=0
